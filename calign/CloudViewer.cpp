...
void CloudViewer::view2(pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud) {
  if (cloud->points.size() > 0) { 
    if (!_is_init) {
      initialize();
      _is_init = true;
      ...
      _viewer->updatePointCloud(cloud, "cloud");
    }

    _viewer->spinOnce(500);
  }
}

void CloudViewer::view_mesh(pcl::PolygonMesh mesh) {
  if (!_is_init) {
    initialize();
    _is_init = true;
  }
  _viewer->removePolygonMesh("mesh");
  _viewer->addPolygonMesh(mesh, "mesh");
