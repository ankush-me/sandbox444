...
def follow_body_traj(pr2, traj, times=None, r_arm = False, l_arm = False, r_gripper = False, l_gripper = False, head = False, base = False):
    ...
    vel_mult = .45 # this was 0.2, if the play-back speed is excessively fast, roll it back to 0.2
    s = 0.3       # this value was 0.5
    vel_l = []
    vel_r = []

    vel_limits = np.r_[pr2.rarm.vel_limits*vel_mult, pr2.larm.vel_limits*vel_mult, pr2.rgrip.vel_limits, pr2.lgrip.vel_limits] # the last two limits are for head and torso
    ...
    #print 'smooth_traj[r_arm]:', smooth_traj['r_arm']
    ...
    if r_gripper:
        ...
        print 'rgrip effort:', pr2.rgrip.get_effort()
    if l_gripper:
        pr2.lgrip.follow_timed_trajectory(times, traj["l_gripper"])
        print 'lgrip effort:', pr2.lgrip.get_effort()
